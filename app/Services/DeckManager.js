SetGameApp.factory('GameManager', function () {
  var game = {};
  game.SetProps = ['color', 'shape', 'shading', 'number'];
  ...
  game.NewGame = function () {
    ...
    shuffle(game.deck);
    while (game.active_cards.length < 12) {
      game.DrawCard();
    }
    game.FindAllSets();
  }
  ...
  //Does a brute force O(n^3) scan of the cards
  //Alot of optimization that could be done here, but for a possible deck size of < 60 and a typical use case of under 20, a more robust solution is not needed.
  game.FindAllSets = function () {
    game.active_sets = [];
    for (var card1 in game.active_cards) {card1 = game.active_cards[card1];
      for (var card2 in game.active_cards) {card2 = game.active_cards[card2];
        for (var card3 in game.active_cards) {card3 = game.active_cards[card3];
          if (card1 != card2 && card2 != card3 && card1 != card3) {
            if (game.GetSets([card1, card2, card3]).length == 4) {
              ...
              //This block excludes duplicates.
              var add = true;
              for (var existingSet in game.active_sets) {existingSet = game.active_sets[existingSet];
  ...
  //Same number of lines as a for loop
  game.DrawCards = function () {
    game.DrawCard();//1
    game.DrawCard();//2
    game.DrawCard();//3
    game.FindAllSets();
